...
**Fork config:** Pre-configured for [Clief Notes](https://www.skool.com/cliefnotes) for an **admin + two moderators** prioritizing **beginner ICM articles**. Scoring profile: Beginner content engagement. See `shared/channel-alignment.md` and `_config/evaluation-criteria.md`.

Community request analysis and workspace delivery pipeline. Takes raw ideas and requests from community DMs (and beginner stuck points), analyzes patterns, evaluates feasibility, prioritizes by impact, and classifies each into MWP-workspace (deliverable as a folder-based workflow) or software-required (needs custom code/app/infrastructure). For this fork, treat **beginner article briefs** as the primary near-term deliverable when an idea is teaching-shaped; MWP/software tracks remain available when classification calls for them.
